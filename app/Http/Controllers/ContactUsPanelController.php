...
use App\Models\ContactUsPanel;
use Illuminate\Support\Facades\Storage;
class ContactUsPanelController extends Controller
{
    //
    ...
    public function store(Request $request)
    {
        ...
        $filePath = $file->store('public/contactus');

        // Create a record in the database for the stored file
        $newFile = new ContactUsPanel();
    ...
    {
        $file = ContactUsPanel::latest()->first();

        if($file){
            $filePath = Storage::disk('public_contactus')->path($file->filename);
